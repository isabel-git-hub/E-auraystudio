...
public class MemberController {
	...
	/*
	 * @PostMapping("/findpw") public String findpw(Model model, MemberDto dto) {
	 * String msg = "";
	 * 
	 * boolean ck = memberService.checkMember(dto); if(ck) msg = "ok"; else { msg =
	 * "error"; model.addAttribute("msg" ,msg); return "findPwForm"; }
	 * 
	 * // 권한 사용자 설정 dto.setPermit(0); // 임시 패스워드 생성 String tmppw =
	 * UUID.randomUUID().toString().substring(0,8); // 임시 패스워드 설정
	 * dto.setUserpw(tmppw); memberService.editUser(dto);
	 * 
	 * ck = mailService.makeMsgTmpPw(dto); if(!ck) { msg =
	 * "메일전송에 실패했습니다. 잠시후 다시 시도해 주세요."; } msg =
	 * "메일을 통해 임시 비밀번호를 전송했습니다. 메일을 확인해 주세요."; model.addAttribute("msg" ,msg);
	 * return "sendmsg"; }
	 * 
	 * @GetMapping("/findId") public String findId() { return "findIdForm"; }
	 * 
	 * @PostMapping("/findId") public String findId(Model model, MemberDto dto) {
	 * String msg = "";
	 * 
	 * String userid = memberService.getMemberById(dto);
	 * 
	 * if(userid == null) msg = "해당하는 아이디가 존재하지 않습니다.!!!"; else msg = "당신의 아이디는 " +
	 * userid + "입니다."; model.addAttribute("msg" ,msg); return "sendmsg"; }
	 */
